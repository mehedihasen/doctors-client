...
const AvaliableAppoint = ({date}) => {
    ...
    return (
        <section className='container mt-5'>
            <h2 className="text-center text-brand mb-5 text-primary">Available Appointments on {date.toDateString()}</h2>
            <div className="row">
                {
                   CardData.map(data=><AppointmentCard date ={date} data ={data} key = {data.id}></AppointmentCard>) 
                }
            </div>
        </section>
    );
